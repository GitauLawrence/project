...
// Get property images
$propertyImages = getPropertyImages($conn, $property_id);

// Get property amenities
$propertyAmenities = getPropertyAmenities($conn, $property_id);
$selectedAmenities = array_map(function($amenity) {
    return $amenity['id'];
}, $propertyAmenities);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($title) || empty($price) || empty($area) || empty($description)) {
        ...
        if (isset($_FILES['main_image']) && $_FILES['main_image']['error'] == 0) {
            $newMainImagePath = uploadImage($_FILES['main_image'], '../assets/images/properties/');
            if ($newMainImagePath) {
        ...
        if (empty($error)) {
            // Update property
            $stmt = $conn->prepare("UPDATE properties SET title = ?, type = ?, purpose = ?, price = ?, bedrooms = ?, bathrooms = ?, area = ?, garages = ?, location_id = ?, year_built = ?, description = ?, main_image = ?, featured = ?, updated_at = NOW() WHERE id = ?");
            $stmt->bind_param("sssdiidiisssii", $title, $type, $purpose, $price, $bedrooms, $bathrooms, $area, $garages, $location_id, $year_built, $description, $mainImagePath, $featured, $property_id);
            ...
            if ($stmt->execute()) {
                ...
                if (isset($_POST['remove_images']) && is_array($_POST['remove_images'])) {
                    foreach ($_POST['remove_images'] as $image_id) {
                        $imgStmt = $conn->prepare("DELETE FROM property_images WHERE id = ?");
                ...
                // Update amenities - First remove all existing
                $delStmt = $conn->prepare("DELETE FROM property_amenities WHERE property_id = ?");
...
<head>
    ...
    <title>Edit Property - SELLAM Real Estate</title>
    ...
    <div class="admin-container">
        ...
        <div class="admin-content">
            ...
            <main class="admin-main">
                ...
                <form class="admin-form" action="edit-property.php?id=<?php echo $property_id; ?>" method="POST" enctype="multipart/form-data">
                    <div class="form-grid">
                        <div class="form-section">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="purpose" name="purpose" required>
                                        ...
                                        <option value="buy" <?php echo ($property['purpose'] == 'buy') ? 'selected' : ''; ?>>For Sale</option>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" id="price" name="price" min="0" value="<?php echo $property['price']; ?>" required>
                            </div>
                            
                            <div class="form-row">
                                <div class="form-group" id="bedrooms-group" style="<?php echo ($property['type'] == 'commercial') ? 'display:none;' : ''; ?>">
                                    <label for="bedrooms">Bedrooms</label>
                                    <input type="number" id="bedrooms" name="bedrooms" min="0" value="<?php echo $property['bedrooms']; ?>">
                                </div>
                                
                                <div class="form-group" id="bathrooms-group" style="<?php echo ($property['type'] == 'commercial') ? 'display:none;' : ''; ?>">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="number" id="area" name="area" min="0" value="<?php echo $property['area']; ?>" required>
                                </div>
                                
                                <div class="form-group" id="garages-group" style="<?php echo ($property['type'] == 'commercial') ? 'display:none;' : ''; ?>">
                                    <label for="garages">Garages</label>
                                    <input type="number" id="garages" name="garages" min="0" value="<?php echo $property['garages']; ?>">
                                </div>
                            </div>
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <select id="location" name="location" required>
                                        ...
                                        <?php foreach ($locations as $location): ?>
                                            <option value="<?php echo $location['id']; ?>" <?php echo ($property['location_id'] == $location['id']) ? 'selected' : ''; ?>><?php echo $location['name']; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" id="year_built" name="year_built" min="1900" max="<?php echo date('Y'); ?>" value="<?php echo $property['year_built']; ?>">
                                </div>
                            </div>
                            
                            <div class="form-group">
                                <label for="description">Description *</label>
                                <textarea id="description" name="description" rows="6" required><?php echo $property['description']; ?></textarea>
                            </div>
                            
                            <div class="form-group checkbox-group">
                                <input type="checkbox" id="featured" name="featured" value="1" <?php echo ($property['featured'] == 1) ? 'checked' : ''; ?>>
                                                ...
                                                <div class="gallery-item">
                                                    ...
                                                    <div class="image-actions">
                                                        <label class="checkbox-container">
                                                            <input type="checkbox" name="remove_images[]" value="<?php echo $image['id']; ?>">
                            ...
                            <div class="amenities-list">
                                <?php foreach ($amenities as $amenity): ?>
                                    <div class="amenity-checkbox">
                                        <input type="checkbox" id="amenity-<?php echo $amenity['id']; ?>" name="amenities[]" value="<?php echo $amenity['id']; ?>" <?php echo (in_array($amenity['id'], $selectedAmenities)) ? 'checked' : ''; ?>>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="manage-properties.php" class="btn-secondary">Cancel</a>
                    </div>
                </form>
            </main>
